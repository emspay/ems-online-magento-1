...
final class Client
{
    ...
    public function createOrderRefund(
        ...
        return $this->postOrderRefund(
            $orderId,
            [
                'amount' => ($amount !== null) ? $amount : null,
                'order_lines' => ($orderLines !== null) ? $orderLines : null
            ]
        );
    }

    /**
     * Post refund data to Ginger API.
     *
     * @param $orderId
     ...
     * @return array
     */
    public function postOrderRefund($orderId, array $refundData)
    {
        try {
            $response = $this->getHttpClient()
                ->method(Http::POST)
                ->uri($this->baseUrl . 'orders/' . $orderId . '/refunds/')
                ->timeout(30)
                ->sends(Mime::JSON)
                ->body(ArrayFunctions::withoutNullValues($refundData))
                ->send();
